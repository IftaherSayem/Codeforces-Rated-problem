...
int main()
{
    ...
    while(n--)
    {
        ...
        string s;
        cin >> s >> a;
        int caph=stoi(s.substr(0,2));
        int capm=stoi(s.substr(3,2));
        //cout << caph << sp << capm << el;
        int minute=caph*60+capm;
        vector<bool>chk(1440,false);
        int co=0;
        while(!chk[minute])
        {
            chk[minute]=true;
            ...
            string s1=to_string(conh);
            if(s1.size()==1) s1="0"+s1;
            string s2=to_string(conm);
            if(s2.size()==1) s2="0"+s2;
            if(s1[0]==s2[1] && s1[1]==s2[0])
            {
                co++;
            }
            minute=(minute+a)%1440; /// because highest is 24 hr
        }
        cout << co << el;
    }
}
