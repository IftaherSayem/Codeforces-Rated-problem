...
int main()
{
    ...
    while(n--)
    {
        ...
        sort(ar.begin(),ar.end(),sor);
        for (int i=1;i<a;i++)
        {
            ll mod=ar[i].second%ar[i-1].second;
            if(mod!=0)
            {
                //ll xd=(ar[i].second/ar[i-1].second+1)*ar[i-1].second;
                ll ans=ar[i-1].second-mod;
                br.push_back({ar[i].first,ans});
                ar[i].second+=ans;
            }
        }
        cout << br.size() << el;
